...
>>>>>>> Stashed changes
	...
	hT = CreateFileMapping(INVALID_HANDLE_VALUE, NULL, PAGE_READWRITE, 0, sizeof(Movimento), TEXT("MovementMemory"));
	if (hT == NULL)
	{
...
{
	_tprintf(TEXT("Mensagem 11111111111111111111111111111111111  Recebida: %s\n"), strJogador);
...
<<<<<<< Updated upstream
		//LER E ESCREVER
		_tprintf(TEXT("[RECEIVED LOGIN] Avião foi registado: %s \n"), ptrMemoria);

		//	if (flag = verificaJogador((*ptrMemoria)[i % 10]) == 1) // todo MUDAR O VERIFICADOR 
	//	{
			//CLIENTE VERIFICADO, ESCREVER NO BUFFER UM BOOLEAN PARA O CLIENTE SABER
			//TCHAR string[BUFFERSIZE];
			//_stprintf_s(string, BUFFERSIZE, TEXT("%s %d"), ((*ptrMemoria)[i % 10]));
			//_stprintf_s((*ptrMemoria)[i % BUFFERS], BUFFERSIZE, &string, i);
			//	}
	//	else
	//	{
...
>>>>>>> Stashed changes
	...
	_tprintf(TEXT("[DEBUG] Terminating recebe_logins()\n"));
	Sleep(250);
}
...
<<<<<<< Updated upstream
		...
		while (true) {
			...
			WaitForSingleObject(hThreadLogins,3000);
=======
...
>>>>>>> Stashed changes
			...
			WaitForSingleObject(htDadosJogo,3000);
